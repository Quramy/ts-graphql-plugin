...
describe(FragmentRegistry, () => {
  ...
  describe(FragmentRegistry.prototype.getExternalFragments, () => {
    it('should return empty array when target document can not be parsed', () => {
      ...
      registry.registerDocument('main.ts', '0', ['fragment X on Query {']);
    ...
    it('should return dependent fragment definitions', () => {
      ...
      registry.registerDocument('main.ts', '0', ['fragment FragmentB on Query { ...FragmentA }']);
    ...
    describe('cache', () => {
      it('should not use cached value when dependent fragment changes', () => {
        ...
        registry.registerDocument('main.ts', '0', ['fragment FragmentX on Query { ...FragmentA }']);
        registry.getExternalFragments('fragment FragmentB on Query { ...FragmentA }', 'main.ts', 0);
      ...
      it('should not use cached value when FragmentSpread set in target documentString changes', () => {
        ...
        registry.registerDocument('main.ts', '1', ['fragment FragmentX on Query { ...FragmentA, ...FragmentB }']);
      ...
      it('should use cached value', () => {
        ...
        registry.getExternalFragments('fragment FragmentB on Query { ...FragmentA }', 'main.ts', 0);
        registry.registerDocument('main.ts', '1', ['fragment FragmentX on Query { ...FragmentA, __typename }']);
        expect(logger).toBeCalledTimes(0);
