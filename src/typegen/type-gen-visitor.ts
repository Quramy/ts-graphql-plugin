...
export class TypeGenVisitor {
  visit(documentNode: DocumentNode, schema: GraphQLSchema, { outputFileName }: VisitOption) {
      ...
      OperationDefinition: {
        enter: node => {
          if (node.operation === 'query') {
            parentTypeStack.stack(schema.getQueryType()!);
            resultFieldElementStack.stack();
          } else if (node.operation === 'mutation') {
            parentTypeStack.stack(schema.getMutationType()!);
            resultFieldElementStack.stack();
          } else if (node.operation === 'subscription') {
            parentTypeStack.stack(schema.getSubscriptionType()!);
            resultFieldElementStack.stack();
          }
