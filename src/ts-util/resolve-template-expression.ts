...
export function createResultForNoSubstitution(node: ts.NoSubstitutionTemplateLiteral, fileName: string): ResolveTemplateExpressionResult {
...
export function resolveTemplateExpression({
  ...
  fileName,
  languageService,
}: ResolveTemplateExpressionOption) {
  ...
  const getSourcePositions: [(pos: number, next: ComputePosition) => ReturnType<ComputePosition>] = [getSourcePosForHead];
